...
void get_params(e *E){
  ...
  //FILE  *gridrXML;
	...
	E->ens_vortexParameters = malloc2d_float(E->n_ens, E->track_length);
...
void _parseInputFile_cycloneData(e *E, xmlDocPtr doc, xmlNodePtr cur){
	...
	while (cur != NULL) {
        ...
        else if((!xmlStrcmp(cur->name, (const xmlChar *) "vortexParameters"))){// polygon
			_parseInputFile_vortexParameters(E, doc, cur);
		}
		cur = cur->next;
	}
}
...
void _parseInputFile_vortexParameters(e *E, xmlDocPtr doc, xmlNodePtr cur){
    ...
    cur = cur->xmlChildrenNode;
	while (cur != NULL) {
		if((!xmlStrcmp(cur->name, (const xmlChar *) "x"))){// coordinates of polygon
			key = lr_pack(xmlNodeListGetString(doc, cur->xmlChildrenNode, 1));

			sscanf((char*)key,"%f\n", &value);

			E->ens_vortexParameters[E->nEnsemble][E->track_count] = value;
